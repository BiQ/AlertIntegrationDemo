...
namespace ChangeReader
{
    public class ReadingHelpers
    {
        ...
        public static DateTimeOffset ReadBookmark()
        {
            ...
            /* An alternative to a local bookmark is to fetch the bookmark from the cloud with the optional BiQ helper method. 
             
            HttpClient client = CreateAlertChangesHttpClient();
            string url = $"approvedchanges-bookmark/{ConfigValues.TenantId}";
            var response = await client.GetAsync(url);
            if (response.StatusCode.Equals(HttpStatusCode.NotFound))
            {
                await SaveBookmark(DateTimeOffset.MinValue);
                return DateTimeOffset.MinValue;
            }
            response.EnsureSuccessStatusCode();
            BookmarkContainer? bookmarkResponce = BiQ.AlertIntegrationDemo.JsonConverter.DeserializeAs<BookmarkContainer>(
                await response.Content.ReadAsStringAsync());
            return bookmarkResponce!.Bookmark;
            */
        }

        public static void SaveBookmark(DateTimeOffset timestamp)
        {
            string jsonString = JsonConverter.Serialize(timestamp);
            File.WriteAllText(bookmarkFileName, jsonString);
            
            /* An alternative to a local bookmark is to save the bookmark to the cloud with the optional BiQ helper method. 
             
            HttpClient client = CreateAlertChangesHttpClient();
            string url = $"approvedchanges-bookmark/{ConfigValues.TenantId}";
            var jsonContent = BiQ.AlertIntegrationDemo.JsonConverter.Serialize(new { bookmark = value });
        ...
        public static string? MapCustomerCategory(string? customerCategoryText)
        {
        ...
        public static void HandleOneChange(ApprovedChange ac)
        {
            // Handle create new
            if (ac.EventType!.Equals("new-person") || ac.EventType!.Equals("new-company"))
            {
                BiQ.AlertIntegrationDemo.DtoTenant.Customer newCustomer = new()
                {
                ...
                BiQ.AlertIntegrationDemo.CustomerSystem.Db.InsertCustomer(newCustomer);
                return;
            }

            // Handle updates
            var dbCustomer = BiQ.AlertIntegrationDemo.CustomerSystem.Db.GetCustomer(ac.CustomerNumber);
            ...
            BiQ.AlertIntegrationDemo.DtoTenant.Customer c = dbCustomer!;
            ...
            BiQ.AlertIntegrationDemo.CustomerSystem.Db.UpdateCustomer(c);
        }
